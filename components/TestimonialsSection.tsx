...
export default function TestimonialsSection({ tone = "white" }: TestimonialsSectionProps) {
  return (
    <section className={`${sectionToneClasses[tone]} ${sectionY}`}>
      <div className={container}>
        <div className={`mx-auto flex max-w-2xl flex-col items-center text-center ${sectionIntroMarginCompact}`}>
          <SectionHeading
        ...
        <div className={`grid ${gridGap} md:grid-cols-3`}>
          {testimonials.map((testimonial) => (
            ...
            >
              <div className="flex gap-1 text-accent-text" aria-label="5 out of 5 stars">
                {Array.from({ length: 5 }).map((_, i) => (
